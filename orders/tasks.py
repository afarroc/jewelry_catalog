...
def _send_order_confirmation_email(order_id):
    ...
    try:
        ...
        logger.info(f"Confirmation email sent for order {order.order_number}: {r.get('id')}")
...
def _send_order_cancellation_email(order_id):
    ...
    try:
        ...
        logger.info(f"Cancellation email sent for order {order.order_number}: {r.get('id')}")
    except Exception as exc:
        logger.error(
